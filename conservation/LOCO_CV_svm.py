...
for i in range(1, 30):
    ...
    for chrom in range(1, 22):
        ...
        X_test = datasetTest.drop(["driver_status", "chrom", "pos", "reference_allele", "alternate_allele", "start"], axis=1) # test dataset X are the signal values ONLY
        y_test = datasetTest["driver_status"] # test dataset y, or labels, are the driver status ONLY
        ...
        datasetTrain = pd.concat([datasetTrain[datasetTrain["driver_status"] == 1].sample(2000), datasetTrain[datasetTrain["driver_status"] == -1].sample(2000)])
        datasetTrain = shuffle(datasetTrain)
        datasetTrain = datasetTrain.reset_index(drop = True)

        X_train = datasetTrain.drop(["driver_status", "chrom", "pos", "reference_allele", "alternate_allele", "start"], axis=1) # train dataset X are the signal values ONLY
        y_train = datasetTrain["driver_status"] # train dataset y, or labels, are the driver status ONLY
