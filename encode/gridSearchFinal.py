...
def getOptimalParams(csv, variant_anno):
    ...
    df = pd.DataFrame({"overallFeatureCat": "histone_chip_seq", "featureGroup": ["consequences"], "model": ["SVM"], "bestParams":[gridSearchRes[0]], "PA_bestScore": [round(gridSearchRes[1], 4)],"LOCO_weightedAv": ["N/A"]})
    dfFin = dfFin.append(df)

    # gradient boosting grid search
    dfFin = dfFin
    gridSearchRes = gridSearchGB(dataset)
    df = pd.DataFrame({"overallFeatureCat": "histone_chip_seq", "featureGroup": ["consequences"], "model": ["GB"], "bestParams":[gridSearchRes[0]], "PA_bestScore": [round(gridSearchRes[1], 4)],"LOCO_weightedAv": ["N/A"]})
    dfFin = dfFin.append(df)
    return dfFin
